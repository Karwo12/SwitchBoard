...
internal static class ActionValidationService
{
    ...
    public static string GetMessage(ActionItemViewModel action) =>
        action.Type == ActionTypeIds.Comment
            ? string.Empty
    ...
    {
        if (action.Type == ActionTypeIds.Comment) return ValidationSeverity.Valid;
    ...
    private sealed class ProgramRunActionValidator : LocalizedValidator
    {
        public override string? Validate(ActionItemViewModel action)
        {
            ...
            if (action.IsUriTarget && !IsProtocolTarget(action.Target))
                return Text(action, "Validation.ProgramUriTarget");
            var requiresProcess = action.IsUriTarget &&
                (action.ChangeAffinity || action.ShouldChangeProcessPriority || action.ShouldChangeMemoryPriority ||
                 action.ShouldChangePerformanceMode || action.IsWindowWaitEnabled || action.RestoreBehaviorId == "closeStarted");
            if (requiresProcess && string.IsNullOrWhiteSpace(action.ProcessName))
                return Text(action, "Validation.PostLaunchProcess");
            if (action.InstanceBehavior == InstanceBehaviorIds.RestartExisting)
            {
                if (action.IsUriTarget && string.IsNullOrWhiteSpace(action.ProcessName))
                    return Text(action, "Validation.UriRestartProcess");
                if (action.IsUriTarget && !string.IsNullOrWhiteSpace(action.ExecutablePath) &&
                    !IsFullExecutablePath(action.ExecutablePath))
                    return Text(action, "Validation.UriRestartPath");
                if (action.IsExecutableTarget && !IsFullExecutablePath(action.Target))
                    return Text(action, "Validation.ProgramRestartPath");
            }
            if ((action.ChangeAffinity || action.ShouldChangeProcessPriority || action.ShouldChangeMemoryPriority || action.ShouldChangePerformanceMode) &&
                ...
                return Text(action, "Validation.PostLaunchProcess");
            if (action.RestoreBehaviorId == "closeStarted" && action.IsExecutableTarget && !IsFullExecutablePath(action.Target))
                return Text(action, "Validation.ProgramRestorePath");
            return null;
        }

        private static bool IsProtocolTarget(string value) => Uri.TryCreate(value, UriKind.Absolute, out var uri) &&
            !uri.IsFile && !string.IsNullOrWhiteSpace(uri.Scheme);
    }
    ...
    private sealed class ScriptActionValidator : LocalizedValidator
    {
        public override string? Validate(ActionItemViewModel action)
        {
            ...
            if (action.RestoreBehaviorId == "closeStarted" && string.IsNullOrWhiteSpace(action.ProcessName))
                return Text(action, "Validation.PostLaunchProcess");
