...
Changes merged into `main` are checked by CI on `windows-latest` with the current .NET 10 SDK. A stable tag such as `v0.4.2` is the release version: the workflow uses `0.4.2` for the application build and creates `SwitchBoard-v0.4.2-win-x64.zip` together with its SHA-256 checksum. Release notes are generated by GitHub.
