    ...
    IDisplayConfirmationService confirmationService) : IReversibleActionHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            guard = DisplayRollbackGuard.Start(previous, GuardTimeout);
            ...
            var keep = await confirmationService.ConfirmAsync(ConfirmationTimeout, cancellationToken);
            if (keep && guard.ProtectionExpired)
            {
                keep = false;
            }
            if (!keep)
