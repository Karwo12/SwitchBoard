    ...
    ILocalizationService localizationService) : IUserDialogService
{
    ...
    public string? SelectArguments(string title, string? initialArguments = null)
        => SelectArgumentsForTarget(title, initialArguments, null);

    public string? SelectArgumentsForTarget(string title, string? initialArguments, string? target)
    {
        var dialog = new ArgumentsPickerWindow(initialArguments ?? string.Empty, target, localizationService)
        {
            Title = title,
