...
public sealed class DisplayActionTests : RuntimeTestBase
{
    ...
    [Fact]
    [Trait("Category", "Unit")]
    public async Task DisplayConfigure_SkipConfirmation_AppliesWithoutDialog()
    {
        var previous = SimulatedPrevious();
        var manager = new TestDisplayManager(previous);
        var action = SimulatedTarget();
        action.Parameters[ActionParameterNames.SkipDisplayConfirmation] = true;
    ...
    [Fact]
    [Trait("Category", "Unit")]
    public async Task DisplayConfigure_Restore_SkipConfirmationAlsoAvoidsDialog()
    {
        var previous = SimulatedPrevious();
        var manager = new TestDisplayManager(previous);
        var action = SimulatedTarget();
        action.Parameters[ActionParameterNames.SkipDisplayConfirmation] = true;
        var handler = new DisplayConfigureActionHandler(manager, new TestDisplayConfirmationService(false));
        var captured = await handler.CaptureStateAsync(action, new(Guid.NewGuid(), Guid.NewGuid()), CancellationToken.None);

        var executed = await handler.ExecuteAsync(action, new(Guid.NewGuid(), Guid.NewGuid()), CancellationToken.None);
        var restored = await handler.RestoreAsync(action, captured!, new(Guid.NewGuid(), Guid.NewGuid()), CancellationToken.None);

        Assert.True(executed.IsSuccessful);
